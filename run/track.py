...
def track(video_dir, clip_dir, video_file, net, ctx, every=25, buffer=25, boxes=False, separate=0, threshold=0.5):
            ...
            if scores[0] > threshold:  # we found a box
                ...
                if len(trackers) == 0:
                    for box in boxes:
                        ...
                        t.start_track(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), rect)
                        # labels.append(label)
                        trackers.append(t)
                else:
                    for (t, l) in zip(trackers, labels):
                        ...
                        # object
                        t.update(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
                        pos = t.get_position()

                        # unpack the position object
                        tr_bboxes.append([int(pos.left()), int(pos.top()), int(pos.right()), int(pos.bottom())])
                last_found = current
                if boxes:
...
def subclipper(video_dir, model_path, every=25, gpus='', buffer=25, boxes=False, separate=0, threshold=0.5):
    ...
    for i, video_file in enumerate(vids):
        ...
        if video_file[-4:] not in file_types:
            ...
            continue
        out, total = track(video_dir=os.path.join(video_dir, 'unprocessed', gpus),
