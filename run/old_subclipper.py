            ...
            if scores[0] > threshold:  # we found a box
                ...
                if not writing:
                    if summary_clip is None and separate < 2:
                        print("Creating Summary Clip: %s_summary.mp4" % (os.path.join(clip_dir, video_file[:-4]), clip_count+1))
                    ...
                    # start the clip
                    if separate > 0:
                        print("Creating Subclip: %s_%05d.mp4" % (os.path.join(clip_dir, video_file[:-4]), clip_count+1))
